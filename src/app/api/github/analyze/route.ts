...
import { analyzeGithubResponseSchema } from "@/features/github/schema";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json(
      analyzeGithubResponseSchema.parse({
        profileId,
        username: githubInput.username,
        stats,
        analysis,
    ...
    if (error instanceof z.ZodError) {
      return NextResponse.json(
        { error: "GitHub analysis returned an invalid structure. Try again." },
        { status: 502 },
      );
    }
